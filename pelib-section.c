...
size_t serialize_section(const pelib_section_t* section, uint8_t* buffer, size_t offset) {
  ...
  memcpy(buffer + section->pointer_to_raw_data, section->contents, section->size_of_raw_data);

  end:
  if (section->pointer_to_raw_data > offset) {
    return section->pointer_to_raw_data + section->size_of_raw_data;
  } else {
...
size_t deserialize_section(const uint8_t* buffer, size_t offset, const size_t size, pelib_section_t* section) {
  ...
  if (section->pointer_to_raw_data + section->size_of_raw_data > size) {
    return 0;
  }

  section->contents = malloc(section->size_of_raw_data);
  memcpy(section->contents, buffer + section->pointer_to_raw_data, section->size_of_raw_data);
